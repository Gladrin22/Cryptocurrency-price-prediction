...
START = "2010-01-01"    # fixing the starting date      data frequencies !(From START to TODAY)

TODAY = date.today().strftime("%Y-%m-%d")  #Until today 


st.title('Cryptocurrency price prediction')   #Title of our web-app
st.write('(Bitcoin, Ethereum, Doge Coin,Lite Coin ,Binance Coin)')

coins = ('BTC-USD','ETH-USD','DOGE-USD','LTC-USD','BNB-USD')  

selected_coins = st.selectbox('Select Dataset for prediction',coins)   

n_years = st.slider("Years of prediction :",1,10)   #Number of years to do the prediction 
period = n_years * 365
...
data_load_state.text('Successfully Predicted !')
